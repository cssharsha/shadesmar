...
void debugUnprojectReproject(const core::types::Keypoint& map_keypoint,
    ...
    for (size_t target_idx = 0; target_idx < observations.size(); ++target_idx) {
        ...
        if (!target_obs.keyframe->hasColorImage()) {
            LOG(WARNING) << "Target keyframe " << target_obs.keyframe->id
                         << " has no color image, skipping";
            continue;
        }
        ...
        for (size_t source_idx = 0; source_idx < observations.size(); ++source_idx) {
            ...
            try {
                ...
                Eigen::Vector3d target_ray_camera =
                    source_camera_T_target_camera.rotation() * source_ray_camera;

                // Transform ray origin (source camera center in target camera frame)
                Eigen::Vector3d source_camera_center_in_target =
                    source_camera_T_target_camera.translation();

                // Step 3: Project ray onto target camera's image plane
                // Ray equation: P(t) = origin + t * direction
                // Image plane: Z = focal_length (approximately, for simplicity we use Z = 1 after
                // normalization)
                ...
                Eigen::Vector3d intersection_3d =
                    source_camera_center_in_target + t * target_ray_camera;
                ...
                LOG(INFO) << "Ray from KF " << source_obs.keyframe->id << " → KF "
                          << target_obs.keyframe->id << ": intersection (" << ray_intersection.x
                          << "," << ray_intersection.y << ") error: " << ray_error << "px";
    ...
    // Create grid image
    cv::Mat grid_image =
        cv::Mat::zeros(rows * target_cell_height, cols * target_cell_width, CV_8UC3);
    ...
    for (int i = 0; i < valid_keyframes.size(); ++i) {
        ...
        cv::circle(resized_image, scaled_original, circle_radius + 1, cv::Scalar(0, 255, 0),
                   line_thickness);
        ...
        for (size_t j = 0; j < kf_data.ray_intersections.size(); ++j) {
            ...
            cv::circle(resized_image, scaled_intersection, circle_radius, cv::Scalar(0, 0, 255),
                       line_thickness);

            // Draw purple line from original to intersection
            cv::line(resized_image, scaled_original, scaled_intersection, cv::Scalar(255, 0, 255),
                     line_thickness);
        ...
        cv::Rect cell_rect(col * target_cell_width, row * target_cell_height, target_cell_width,
                           target_cell_height);
    ...
    std::string title = "Map Keypoint " + std::to_string(map_keypoint.id()) +
                        " - Ray Consistency Across " + std::to_string(valid_keyframes.size()) +
                        " Keyframes";
    cv::putText(grid_image, title, cv::Point(10, 25), cv::FONT_HERSHEY_SIMPLEX, 0.8,
                cv::Scalar(255, 255, 0), 2);

    // Add legend
    cv::putText(grid_image,
                "Green: Original Pixel | Red: Ray Intersections | Purple: Consistency Lines",
                cv::Point(10, grid_image.rows - 15), cv::FONT_HERSHEY_SIMPLEX, 0.5,
    ...
    // Add statistics
    std::string stats_text = "Overall: " + std::to_string(total_rays) +
                             " rays, Avg: " + std::to_string(static_cast<int>(overall_avg_error)) +
                             "px, Max: " + std::to_string(static_cast<int>(overall_max_error)) +
                             "px";
    cv::putText(grid_image, stats_text, cv::Point(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.6,
                cv::Scalar(255, 255, 0), 1);

    // Save debug image
    std::string filename =
        "/data/robot/unproject_gtsam_" + std::to_string(map_keypoint.id()) + ".png";
    ...
    LOG(INFO) << "Overall statistics: " << total_rays << " rays, Avg error " << overall_avg_error
              << "px, Max error " << overall_max_error << "px across " << valid_keyframes.size()
              << " keyframes";
}

std::vector<ObservationData> buildObservationData(const core::types::Keypoint& map_keypoint,
    ...
    for (const auto& location : map_keypoint.locations) {
        ...
        try {
            obs.camera_pose_in_odom =
                computeCameraPoseInOdom(*keyframe, tft, obs.camera_frame_id, base_link_frame_id);
...
Eigen::Isometry3d computeCameraPoseInOdom(const core::types::KeyFrame& keyframe,
                                          const stf::TransformTree& tft,
