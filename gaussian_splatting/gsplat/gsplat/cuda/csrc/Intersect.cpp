...
std::tuple<at::Tensor, at::Tensor, at::Tensor> intersect_tile(
    ...
    LOG(INFO) << "Getting here in the interset tile";
    DEVICE_GUARD(means2d);
    LOG(INFO) << "Finished device gaurd";
    CHECK_INPUT(means2d);
    CHECK_INPUT(radii);
    CHECK_INPUT(depths);
    LOG(INFO) << "Finished checking the inputs";

    auto opt = depths.options();
    uint32_t n_elements = means2d.numel() / 2;
    bool packed = means2d.dim() == 2;
    LOG(INFO) << "Some more random checks";
    if (packed) {
        TORCH_CHECK(
    ...
    at::Tensor offsets;
    LOG(INFO) << "Radii: " << radii.dtype() << " means: " << means2d.dtype()
              << " depths: " << depths.dtype();
    if (n_elements) {
        LOG(INFO) << "Launchihng the interset kernel " << means2d.sizes() << " "
                  << radii.sizes() << " " << depths.sizes();
        ...
        if (segmented) {
            ...
            offsets = at::cat({at::tensor({0}, opt.dtype(at::kInt)), offsets});
        }
    } else {
        n_isects = 0;
    }
    LOG(INFO) << "Finished launching the kernel";
    ...
    if (n_isects) {
        LOG(INFO) << "Calling the second pass kernel";
        launch_intersect_tile_kernel(
            // inputs
            means2d,
            radii,
...
at::Tensor intersect_offset(
    ...
    at::Tensor offsets =
        at::empty({I, tile_height, tile_width}, opt.dtype(at::kInt));
