...
public:
    explicit RerunTrainingVisualizer(
        const std::string& recording_id = "gaussian_splat_training",
    ...
    void visualizeKeyframeBatch(const training::KeyframeBatch& batch, const std::string& stage = "training");
    void visualizeCurrentSplats(const std::vector<core::types::GaussianSplat>& splats, uint32_t iteration);
    ...
    // Configuration
    void setVisualizationFrequency(uint32_t log_every_n_batches) { log_frequency_ = log_every_n_batches; }
    void enableBatchVisualization(bool enabled) { visualize_batches_ = enabled; }
    void enableSplatVisualization(bool enabled) { visualize_splats_ = enabled; }
...
private:
    ...
    void logPoints3D(const std::string& entity_path, const std::vector<core::types::GaussianSplat>& splats);
    
    // Batch visualization helpers
    void visualizeBatchImages(const training::KeyframeBatch& batch, const std::string& base_path);
    void visualizeBatchCameraPoses(const training::KeyframeBatch& batch, const std::string& base_path);
