...
__global__ void intersect_tile_kernel(
    ...
    // printf("Index: %u, %d, %u, %u\n", idx, packed, nnz, I * N);
    bool first_pass = cum_tiles_per_gauss == nullptr;
    if (idx >= (packed ? nnz : I * N)) {
        return;
    }

    // printf("Radii access: %d\n", idx * 2);
    // printf("Radii access: %d\n", idx * 2 + 1);
    ...
    tile_min.x =
        min(max(0, (uint32_t)floor(tile_x - tile_radius_x)), tile_width);
    tile_min.y =
        min(max(0, (uint32_t)floor(tile_y - tile_radius_y)), tile_height);
    tile_max.x =
        min(max(0, (uint32_t)ceil(tile_x + tile_radius_x)), tile_width);
    tile_max.y =
        min(max(0, (uint32_t)ceil(tile_y + tile_radius_y)), tile_height);
    ...
    int64_t depth_id_enc =
        static_cast<uint32_t>(depth_i32); // Zero-extend to 64-bit
...
void launch_intersect_tile_kernel(
    ...
    TORCH_CHECK(
        radii.scalar_type() == at::kInt, "radii tensor must be of type int32_t"
    );
    printf("Calling the intersect kernel\n");
    bool packed = means2d.dim() == 2;

    printf("Everything should be packed: %d %u\n", packed, I);
    uint32_t N = 0, nnz = 0;
    // uint32_t N, nnz;
    int64_t n_elements;
    if (packed) {
    ...
    // the first 32 bits are used for the image id and tile id altogether,
    ...
    printf(
        "iids: %d, gid: %d, ctpg: %d, tpg: %d, fid: %d, iid: %d\n",
        image_ids.has_value(),
...
__global__ void intersect_offset_kernel(
    ...
    if (idx > 0) {
        // visit the current and previous isect_id and check if the (bid,
        // cid, tile_id) tuple changes.
