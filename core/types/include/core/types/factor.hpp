...
struct Factor {
    ...
    std::variant<types::Pose, types::Pose> measurement;
    ...
    proto::Factor toProto() const {
        ...
        if (type == proto::FactorType::PRIOR) {
            ...
            *factor_proto.mutable_relative_pose() = std::get<0>(measurement).toProto();
        }
    ...
    static Factor fromProto(const proto::Factor& factor_proto) {
        ...
        if (factor_proto.has_absolute_pose()) {
            ...
            factor.measurement.emplace<0>(types::Pose::fromProto(factor_proto.relative_pose()));
        }

        factor.information = Eigen::Map<const Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>(
