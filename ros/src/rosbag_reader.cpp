...
bool RosbagReader::initialize() {
  ...
  try {
    ...
    std::cout << "Duration: " << metadata.duration.count() / 1e9 << " seconds"
              << std::endl;
...
bool RosbagReader::shouldCreateKeyframe(
  ...
  double dx =
      current_odom.pose.pose.position.x - last_keyframe_pose_.position.x();
  double dy =
      current_odom.pose.pose.position.y - last_keyframe_pose_.position.y();
  double dz =
      current_odom.pose.pose.position.z - last_keyframe_pose_.position.z();
...
template <typename T>
std::shared_ptr<T>
RosbagReader::findClosestMessage(std::deque<std::shared_ptr<T>> &messages,
  ...
  while (!messages.empty()) {
    ...
    double time_diff =
        std::abs((rclcpp::Time(msg->header.stamp) - target_time).seconds());
...
void RosbagReader::processOdometry(
  ...
  auto camera_info_msg =
      findClosestMessage(camera_info_messages_, msg_time, 0.1);
  ...
  if (current_keyframe_id_ > 1) {
    ...
    odom_factor.connected_nodes = {current_keyframe_id_ - 1,
    ...
    relative_pose.position =
        keyframe->pose.position - last_keyframe_pose_.position;
