...
protected:
  ...
  // Helper functions
  core::types::KeyFrame::Ptr createKeyFrame(uint64_t id, double x, double y,
                                            double z) {
  ...
  core::types::Factor
  createOdometryFactor(uint64_t id, uint64_t from_id, uint64_t to_id,
...
TEST_F(GraphStorageIntegrationTest, OptimizeAndStore) {
  ...
  EXPECT_TRUE(
      loaded_kf1->pose.position.isApprox(Eigen::Vector3d(0, 0, 0), 1e-5));
  EXPECT_TRUE(
      loaded_kf2->pose.position.isApprox(Eigen::Vector3d(1, 0, 0), 1e-5));
  EXPECT_TRUE(
      loaded_kf3->pose.position.isApprox(Eigen::Vector3d(1, 1, 0), 1e-5));
}
...
TEST_F(GraphStorageIntegrationTest, IncrementalUpdateAndStore) {
  ...
  EXPECT_EQ(connected_factors.size(),
            2); // Should be connected to both odom factors
}
