...
TEST(TypesTest, KeyFrameWithImageData) {
  ...
  ASSERT_EQ(depth_image.data.type(), CV_32FC1)
      << "Original depth image type not preserved";
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    cv::Mat color_diff;
    cv::compare(orig_color_channels[i], conv_color_channels[i], color_diff,
                cv::CMP_NE);
  ...
  ASSERT_EQ(orig_depth.size(), conv_depth.size())
