...
bool GtsamReconstruct::triangulate(const core::types::KeyFrame& kf1,
    ...
    if (gtsam_csv.is_open()) {
        ...
        Eigen::Isometry3d kf1_world_pose =
            kf1.pose.getEigenIsometry() * get_base_link_tf(kf1.color_data.value().frame_id);
        Eigen::Vector3d kf1_translation = kf1_world_pose.translation();
        Eigen::Vector3d kf1_rpy = stf::getRPY(kf1_world_pose);

        gtsam_csv << std::fixed << std::setprecision(6) << kf1_translation.x() << ","
                  << kf1_translation.y() << "," << kf1_translation.z() << "," << kf1_rpy.x() << ","
                  << kf1_rpy.y() << "," << kf1_rpy.z() << "," << kf1.id << ","
                  << kf1.color_data.value().frame_id << "\n";

        // Write keyframe 2 pose
        Eigen::Isometry3d kf2_world_pose =
            kf2.pose.getEigenIsometry() * get_base_link_tf(kf2.color_data.value().frame_id);
        Eigen::Vector3d kf2_translation = kf2_world_pose.translation();
        Eigen::Vector3d kf2_rpy = stf::getRPY(kf2_world_pose);

        gtsam_csv << std::fixed << std::setprecision(6) << kf2_translation.x() << ","
                  << kf2_translation.y() << "," << kf2_translation.z() << "," << kf2_rpy.x() << ","
                  << kf2_rpy.y() << "," << kf2_rpy.z() << "," << kf2.id << ","
                  << kf2.color_data.value().frame_id << "\n";
...
bool GtsamReconstruct::triangulateFromMapKeypoint(const core::types::Keypoint& map_keypoint,
    ...
    std::vector<ObservationData> observations =
        triangulation_utils::buildObservationData(map_keypoint, map_store, tft, base_link_frame_id);
    ...
    // // DEBUG: Call ray-based debug visualization (works for both successful and failed
    // triangulations) static std::set<uint32_t> debug_generated_keypoints;
    //
    ...
    //     Eigen::Vector3d debug_point = success ? triangulated_point_world :
    //     Eigen::Vector3d::Zero();
    //
    //     triangulation_utils::debugUnprojectReproject(map_keypoint, observations, debug_point,
    //     base_link_frame_id); debug_generated_keypoints.insert(map_keypoint.id());
    //
    //     LOG(INFO) << "DEBUG: Generated GTSAM ray-based visualization for map keypoint " <<
    //     map_keypoint.id()
...
bool GtsamReconstruct::triangulateFromTwoObservationsGtsam(
    ...
    try {
        ...
        // Use GTSAM DLT triangulation
        gtsam::Point3 triangulated_point =
            gtsam::triangulatePoint3(cameras, measurements, gtsam_rank_tolerance, gtsam_optimize);

        // Convert back to Eigen
        triangulated_point_world =
            Eigen::Vector3d(triangulated_point.x(), triangulated_point.y(), triangulated_point.z());

        // GTSAM-specific enhanced validation checks

        // 1. Distance from cameras check
        double dist_to_cam1 =
            (obs1.camera_pose_in_odom.translation() - triangulated_point_world).norm();
        double dist_to_cam2 =
            (obs2.camera_pose_in_odom.translation() - triangulated_point_world).norm();
        if (std::max(dist_to_cam1, dist_to_cam2) > max_distance_from_cameras) {
            LOG(WARNING) << "GTSAM: Point too far from cameras ("
                         << std::max(dist_to_cam1, dist_to_cam2) << "m > "
                         << max_distance_from_cameras << "m)";
            return false;
        }

        // 2. Reprojection error validation
        auto checkReprojectionError = [&](const gtsam::PinholeCamera<gtsam::Cal3_S2>& camera,
                                          const gtsam::Point2& measurement) -> double {
            try {
        ...
        if (max_error > max_reprojection_error) {
            LOG(WARNING) << "GTSAM: High reprojection error (" << max_error << "px > "
                         << max_reprojection_error << "px)";
            return false;
        }

        LOG(INFO) << "GTSAM validation: max_dist=" << std::max(dist_to_cam1, dist_to_cam2)
                  << "m, max_reproj_err=" << max_error << "px";

        // 3. Standard cheirality checks: ensure point is in front of both cameras
        Eigen::Vector3d point_camera1 =
            obs1.camera_pose_in_odom.inverse() * triangulated_point_world;
        if (point_camera1.z() <= 0) {
            LOG(WARNING) << "GTSAM: Point behind first camera";
            return false;
        }

        Eigen::Vector3d point_camera2 =
            obs2.camera_pose_in_odom.inverse() * triangulated_point_world;
        ...
        LOG(INFO) << "GTSAM two-view triangulation successful: "
                  << triangulated_point_world.transpose() << " (time: " << duration.count()
                  << "μs)";
        return true;
...
bool GtsamReconstruct::triangulateFromMultipleObservationsGtsam(
    ...
    try {
        ...
        if (max_distance_to_cameras > max_distance_from_cameras) {
            LOG(WARNING) << "GTSAM multi-view: Point too far from cameras ("
                         << max_distance_to_cameras << "m > " << max_distance_from_cameras << "m)";
            return false;
        }
        ...
        if (max_reprojection_err > max_reprojection_error) {
            LOG(WARNING) << "GTSAM multi-view: High reprojection error (" << max_reprojection_err
                         << "px > " << max_reprojection_error << "px)";
            return false;
        }

        LOG(INFO) << "GTSAM multi-view validation: max_dist=" << max_distance_to_cameras
                  << "m, max_reproj_err=" << max_reprojection_err << "px";
