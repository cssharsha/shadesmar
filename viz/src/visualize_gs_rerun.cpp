...
bool GaussianSplatRerunVisualizer::initializeRerunClient() {
    try {
        ...
        rerun_visualizer_ = std::make_shared<RerunVisualizer>(app_name_, recording_id_, host_, port_);
        ...
        LOG(INFO) << "Successfully initialized Rerun client for Gaussian splat visualization with recording_id: " 
                  << recording_id_;
        return true;
...
void GaussianSplatRerunVisualizer::visualizationWorker() {
    ...
    while (running_) {
        ...
        notification_cv_.wait_for(lock, std::chrono::seconds(1), [this] {
...
void GaussianSplatRerunVisualizer::visualizeLatestSplatBatches() {
    ...
    try {
        ...
        if (!all_splats.empty()) {
            double timestamp = current_timestamp_ > 0 ? current_timestamp_ : 
                              std::chrono::duration_cast<std::chrono::nanoseconds>(
                                  std::chrono::steady_clock::now().time_since_epoch()).count() / 1e9;
            
            rerun_visualizer_->addGaussianSplats(all_splats, "gaussian_splats/all_splats", timestamp);
